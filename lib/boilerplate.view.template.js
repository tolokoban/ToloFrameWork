  ...
  try {
    ...
    links.forEach(function (link) {
      try {
        output.push(
          "new Link({",
          "  A:" + pod2code.call( this, link.A ) + ",",
          "  B:" + pod2code.call( this, link.B ),
          "});");
      }
      catch( ex ) {
...
function pod2CodePath( items, path ) {
  ...
  if( pieces.length === 1 ) {
    ...
    var objCode = "that.$elements." + pieces.shift();
 ...
 * {Behing onValueChanged} -> { action: ["CODE_BEHIND.onValueChanged.call( this, v )"] }
 */
