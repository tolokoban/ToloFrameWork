 ...
 *
 * @param   {objects} def - `{ "bob/foo.png": "", "yo/man.kml": "maps/man.kml" }`
 * @param {object} resources -
 * @param {Source} depFile - Source of the dependency file.
 * @returns {undefined}
 */
function lookForExternalDependenciesRES( def, resources, depFile ) {
    if ( !def ) return;
    for ( const dep of Object.keys( def ) ) {
        ...
        const srcDep = Project.srcOrLibPath( `mod/${dep}` ) || Project.srcOrLibPath( dep );
        if ( !srcDep ) {
            Fatal.fire(
 ...
 *
 * @param   {objects} def - `{ ... }`
 * @param {array} javascriptSources - Array of teh JS files to include in current HTML page.
 * @returns {undefined}
 */
function lookForExternalDependenciesJS( def, javascriptSources ) {
    if ( !def ) return;
