...
function linkForRelease(src, pathJS, pathCSS, options) {
    ...
    var zippedInnerCSS = minifyCSS(addFilePrefix(nameWithoutExt) + ".css", innerCSS, options);
    ...
    PathUtils.file(
        ...
        (options.noZip ? innerJS + "\n" + combination.jsDebug : sourcemapJS.generatedContent())
            + (options.noMap || options.noZip ? ''
...
function combineRequires(output, options) {
    ...
    var cache = {},
        ...
        // Combined unzipped JS.
        combinedUnzipJS = '',
        // Combined unzipped CSS.
        combinedUnzipCSS = '',
    ...
    while (fringe.length > 0) {
        ...
        combinedUnzipJS += '\n\n//########################################\n' + src.tag('src');
        map = src.tag('map');
        currentSourceMapJS = new SourceMap(map, src.tag('zip'));
        ...
        if (src) {
            ...
            combinedUnzipCSS += "*******/\n";
            combinedUnzipCSS += "/*** " + moduleName + ".css ***/\n" + src.tag("src");
    ...
    return {
        ...
        jsDebug: combinedUnzipJS,
        cssDebug: combinedUnzipCSS
    };
}
...
function compileJS(path, options) {
    ...
    if (!src.isUptodate()) {
        ...
        dependencies = findDependencies(minification.zip, src);
        src.tag('src', code);
...
function findDependencies(minifiedCode, src) {
...
function minifyCSS(name, code, options) {
    ...
    less.render(code, {sourceMap: {}, compress: options.noZip ? false : true}, function (e, output) {
        try {
