...
function lookForExternalDependencies( jsFiles ) {
    ...
    for( jsFileName in jsFiles ) {
        ...
        depFileName = Project.srcOrLibPath( depFileName );
        if( depFileName ) {
            depFile = new Source( Project, depFileName );
            try {
                dependencies = JSON.parse( depFile.read() );
                ...
                if( dependencies.js ) {
                    ...
                    dependencies.js.forEach(function ( js ) {
                        var filename = Project.srcOrLibPath( "mod/" + js );
                ...
                if( dependencies.res ) {
                    for( dep in dependencies.res ) {
                        ...
                        if( !srcDep ) {
                            Fatal.fire(
                                "Unable to find dependency file `" + dep + "` not `mod/"
                                    + dep + "`!",
