...
function linkForRelease(src, pathJS, pathCSS, options) {
    ...
    if ( options.dev ) {
      // DEBUG
      // Combine all Javascript and create a sourceMap.
      var concat = new ConcatWithSourcemaps(true, addFilePrefix(nameWithoutExt) + ".js", '\n');
      concat.add( null, "// " + (new Date()));
      concat.add( '@.js', innerJS );
      for (key in combination.js) {
          val = combination.js[key];
          concat.add(key, val.src);
      }
      concat.add( null, "//# sourceMappingURL=" + addFilePrefix(nameWithoutExt) + ".js.map");

      prj.flushContent( "js/" + addFilePrefix(nameWithoutExt) + ".js", concat.content );
      prj.flushContent( "js/" + addFilePrefix(nameWithoutExt) + ".js.map", concat.sourceMap );
      head.children.push(
          {type: Tree.TAG, name: 'script', attribs: {
              defer: null, src: backToRoot + "js/" + addFilePrefix(nameWithoutExt) + ".js"
          }}
      ...
      // Combine all CSS and create a sourceMap.
      concat = new ConcatWithSourcemaps(true, addFilePrefix(nameWithoutExt) + ".css", '\n');
      concat.add( null, "/* " + (new Date()) + " */");
      if (innerCSS.trim().length > 0) {
        concat.add( '@.css', innerCSS );
      }
      for (key in combination.css) {
          val = combination.css[key];
          concat.add(key + ".css", val.src);
      }
      concat.add( null, "/*# sourceMappingURL=" + addFilePrefix(nameWithoutExt) + ".css.map */");

      prj.flushContent( "css/" + addFilePrefix(nameWithoutExt) + ".css", concat.content );
      prj.flushContent( "css/" + addFilePrefix(nameWithoutExt) + ".css.map", concat.sourceMap );
      head.children.push(
      ...
      concat.add( '@.js', stub.zip, stub.map );
      for (key in combination.js) {
          val = combination.js[key];
          concat.add(key, val.zip, val.map);
      }
      concat.add( null, "//# sourceMappingURL=" + addFilePrefix(nameWithoutExt) + ".js.map");
      ...
      concat.add( null, "/* " + (new Date()) + " */");
      if (innerCSS.trim().length > 0) {
        stub = minifyCSS("@.css", innerCSS);
        concat.add( '@.css', stub.zip, stub.map );
      }
      for (key in combination.css) {
          val = combination.css[key];
          concat.add(key + ".css", val.zip, val.map);
      }
      concat.add( null, "/*# sourceMappingURL=" + addFilePrefix(nameWithoutExt) + ".css.map */");
...
function combineRequires(output, options) {
    ...
    while (fringe.length > 0) {
        ...
        if (!jsFiles[moduleName]) {
            jsFiles[moduleName] = { src: src.tag('src'), zip: src.tag('zip'), map: src.tag('map') };
        }
        ...
        if (src) {
            if (!cssFiles[moduleName]) {
                cssFiles[moduleName] = { src: src.tag('src'), zip: src.tag('zip'), map: src.tag('map') };
            }
        }
    }
...
function minifyCSS(name, code, options) {
    ...
    try {
        var css = Util.zipCSS( code, name + ".css" );
        css.warnings.forEach(function (itm) {
          console.log("[WARNING] ".yellowBG.white.bold, name.bold);
          console.log(JSON.stringify(itm, null, '  ').yellow.bold);
        });
        if (css.errors.length > 0) {
          throw css.errors.join("\n");
        }
        css.map.file = name + ".css";
        css.map.sources = [name + ".css"];
        css.map.sourcesContent = [code];
        result = {
            src: css.src,
            zip: css.zip,
            map: css.map,
