...
function compile( file, _options ) {
    try {
        ...
        Fatal.bubble( ex, `[compiler-html2] compile("${file}")` );
    }
}
...
function isUptodate( sourceHTML ) {
    try {
        ...
        Fatal.bubble( ex, `[compiler-html2] isUptodate( ${sourcceHTML.name()} )` );
    }
}
...
function link( src, options ) {
    try {
        ...
        Fatal.bubble( ex, `[compiler-html2] link( ${src.name()} )` );
    }
}
...
function linkForRelease( src, pathJS, pathCSS, options, htmlDir ) {
    try {
        ...
        console.info("[compiler-html2] src=", src);
        ...
        Fatal.bubble( ex, `[compiler-html2] linkForRelease(\n${src.name()}, ${pathJS}, ${pathCSS} )` );
    }
}
...
function combineRequires( output, options ) {
    try {
        ...
        while ( fringe.length > 0 ) {
            ...
            const tags = Project.createTagsManager( moduleName );
            //src = CompilerJS.compile( Project, moduleName, options, output );
            CompilerJS.compile( module, tags, options );
            if ( !jsFiles[ moduleName ] ) {
                jsFiles[ moduleName ] = {
                    src: tags.loadModJs( module.name ),
                    zip: tags.loadModJsZip( module.name ),
                    map: tags.loadModJsMap( module.name ),
                    dep: tags.loadModJsReq( module.name )
                };
            }
            // Adding dependencies to the `fringe`.
            dependencies = tags.loadModJsReq( module.name )
        ...
        Fatal.bubble( ex, `[compiler-html2] combineRequires( ... )` );
    }
}
...
function compileCSS( path, options ) {
    const absPath = Project.srcOrLibPath( path );
    if ( !absPath ) return null;
    const src = new Source( Project, path );
    const tags = Project.createModule( src.name() );
    ...
    if ( !src.isUptodate() ) {
        ...
        tags.saveModCss( module.name, cssCode );
        tags.saveModCssZip( module.name, minify.zip );
        tags.saveModCssMap( module.name, minify.map );
        src.save();
    }
    return src;
}
