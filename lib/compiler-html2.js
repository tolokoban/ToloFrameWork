 ...
 * For each module, check if there is  a folder with the same name. It
...
function link(src, options) {
    ...
    /*if (options.noZip) {
        output = linkForDebug(src, pathJS, pathCSS, options);
    } else {*/
...
function linkForDebug(src, pathJS, pathCSS, options) {
    var nameWithoutExt = src.name().substr(0, src.name().length - 5);
    // If `nameWithoutExt` is in a subfolder, `backToRoot` must contain
    // as many `../` as there are subfolders in `nameWithoutExt`.
    var backToRoot = getBackToRoot(nameWithoutExt);
    var output = src.tag("output");

    embedDebugCSS(nameWithoutExt, output, pathCSS);
    
    return output;
}

function embedDebugCSS(name, output, pathCSS) {
    var key, val;
    var root = output.root;
    var head = findHead(root);
    var innerCSS = concatDicValues(output.innerCSS);
    var filesToIncludeCSS = {};

    filesToIncludeCSS['@' + name + '.css'] = output.innerCSS;

    console.log(output.require);
}
...
function linkForRelease(src, pathJS, pathCSS, options) {
    ...
    // If there is a CSS fie with the same name as the HTML file, embed it.
    ...
    var zippedInnerCSS = minifyCSS(addFilePrefix(nameWithoutExt) + ".css", innerCSS);
    var sourcemapCSS = zippedInnerCSS ? new SourceMap(zippedInnerCSS.map, zippedInnerCSS.zip) : null;
    if (zippedInnerCSS) {
        // Add inner CSS only if it exists!
        sourcemapCSS.append(combination.css);
        PathUtils.file(
            Project.wwwPath("css/map/" + addFilePrefix(nameWithoutExt) + ".css.map"),
            JSON.stringify(sourcemapCSS.sourcemap())
        );
        PathUtils.file(
            Project.wwwPath("css/" + addFilePrefix(nameWithoutExt) + ".css"),
            sourcemapCSS.generatedContent()
                + "\n//# sourceMappingURL=map/" + addFilePrefix(nameWithoutExt) + ".css.map"
        );
        head.children.push(
    ...
    var zippedInnerJS = minifyJS("" + addFilePrefix(nameWithoutExt) + ".js", innerJS, options);
    var sourcemapJS = new SourceMap(zippedInnerJS.map, zippedInnerJS.zip);
    sourcemapJS.append(combination.js);
    PathUtils.file(
        Project.wwwPath("js/map/" + addFilePrefix(nameWithoutExt) + ".js.map"),
        JSON.stringify(sourcemapJS.sourcemap())
    );
    PathUtils.file(
        Project.wwwPath("js/" + addFilePrefix(nameWithoutExt) + ".js"),
        sourcemapJS.generatedContent()
            + "\n//# sourceMappingURL=map/" + addFilePrefix(nameWithoutExt) + ".js.map"
    );
    head.children.push(
...
function writeResources(output) {
    ...
    var resourcePath;
    for (moduleName in output.require) {
        resourcePath = Project.srcOrLibPath("mod/" + moduleName);
        if (resourcePath) {
            // Ok, this folder exists.
            console.info("Copy resource: " + (moduleName + "/").cyan);
            Project.copyFile(resourcePath, Project.wwwPath('css/' + moduleName));
        }
    }
...
function minifyJS(name, code, options) {
    var minification = UglifyJS.minify(code, {
        compress: options.noZip ? false : true,
        fromString: true,
...
function compileJS(path, options) {
    ...
    if (!src.isUptodate()) {
        ...
        minification = minifyJS(moduleShortName + ".js", code, options);
...
function compileCSS(path) {
    ...
    if (!src.isUptodate()) {
        ...
        var minify = minifyCSS(src.name(), cssCode);
        src.tag('src', cssCode);
...
function addFilePrefix(path, prefix) {
    ...
    return path.substr(0, filenameStart) + prefix + path.substr(filenameStart);
}
