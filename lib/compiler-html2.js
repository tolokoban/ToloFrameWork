...
function compileJS(path, options) {
    ...
    if (!src.isUptodate()) {
        ...
        minification = UglifyJS.minify(code, {
            fromString: true,
            outSourceMap: moduleShortName + ".js.map"
        });
        ...
        if (options.noZip) {
            ...
            src.tag('src', code);
            src.tag('code', minification.code);
            sourceMap = JSON.parse(minification.map);
            sourceMap = {
                version: sourceMap.version,
                file: sourceMap.file,
                sources: [moduleShortName + '.js'],
                sourcesContent: [code],
                names: sourceMap.names,
                mappings: sourceMap.mappings
            };
            src.tag('map', JSON.stringify(sourceMap));
        }
