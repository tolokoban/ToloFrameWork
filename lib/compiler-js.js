...
function compileJS( project, path, options ) {
    ...
    if ( options.transpilation ) {
        ...
        console.log( "Transpilation: OFF".red );
...
function processAttributeVar( project, depJSON, watch, depAbsPath ) {
    try {
        ...
        for( const varName of Object.keys( depJSON.var ) ) {
            ...
            if ( !srcVar ) {
                Fatal.fire(
                    ...
                    + `Folder: ${Path.join( folder, varFilename )}`,
                    depAbsPath
                );
            }
        ...
        console.info("[compiler-js] depJSON=", depJSON);
        Fatal.fire( `...processAttributeVar!\n${ex}` );
    }
}
...
function convertIntoModule( src, moduleShortName, head ) {
    const options = {
        name: Util.replaceSlashesWithDots( moduleShortName ),
