...
function Source( prj, _file ) {
    ...
    const
        ...
        file = _file.substr( 0, srcDir.length ) !== srcDir ? _file : _file.substr( srcDir.length );
...
Source.prototype.isUptodate = function () {
    const
        ...
        pathJs = Util.replaceExtension( path, ".js" ),
        pathXjs = Util.replaceExtension( path, ".xjs" ),
        pathIni = Util.replaceExtension( path, ".ini" ),
        pathDep = Util.replaceExtension( path, ".dep" ),
        inputs = [ pathJs, pathXjs, pathIni, pathDep ],
        watch = this.tag( 'watch' ) || [];
    inputs.push( ...watch );
    return !Util.isNewerThan( inputs, pathTmp );

    /*
        if ( !this.existsSync( this._tmpFile ) ) {
            // If no tmp file exists, we must create it, then this source file is out of date.
            return false;
        }
        const
            statSrc = FS.statSync( this._absPath ),
            statTmp = FS.statSync( this._tmpFile );
        if ( statSrc.mtime < statTmp.mtime ) {

            if ( !this.name().endsWith( '.js' ) && !this.name().endsWith( '.xjs' ) ) {
                return true;
            }
            // For JS files, we look for INTL file.
            const
                pathSrc = this.getAbsoluteFilePath(),
                pathIni = pathSrc.substr( 0, pathSrc.length - 2 ) + "ini";
            // Check intl file (*.ini).
            if ( typeof this.tag( "intl" ) === 'string' && this.tag( "intl" ).length === 0 ) {
                ...
                if ( FS.existsSync( pathIni ) ) {
                    ...
                    console.log( "INI file is missing. Creating one: " + pathSrc );
                    FS.writeFileSync( pathIni, "[en]\n\n[fr]\n\n" );
                    return false;
                }
            }
            // For JS files, we look for DEP file.
            var pathDep = pathSrc.substr( 0, pathSrc.length - 2 ) + "dep";
            if ( FS.existsSync( pathDep ) ) {
                var statDep = FS.statSync( pathDep );
                if ( statDep.mtime > statTmp.mtime ) {
                    // Dep file has changed.
                    return false;
                }
            }
            // Look  at watch  files.  There are  files  that produce  the
            // `GLOBAL` variable.
            var watch = this.tag( 'watch' ) || [];
            var srcWatch, statWatch, k;
            for ( k = 0; k < watch.length; k++ ) {
                srcWatch = new Source( this.prj(), watch[ k ] );
                if ( srcWatch.exists() ) {
                    statWatch = FS.statSync( srcWatch.getAbsoluteFilePath() );
                    if ( statWatch.mtime > statTmp.mtime ) {
                        console.log( watch[ k ] + " => ".cyan + this.name() );
                        return false;
                    }
                }
            }
