...
exports.start = function ( pkg ) {
    if ( !isInEmptyFolder() ) {
        ...
        yesno( "Are you sure you still want to continue in this non empty folder?\n" +
            "This will DELETE all the files!".bgRed.white + "  ", stepStart );
    } else {
        stepStart();
    };
};
    ...
    try {
        ...
        if ( FS.existsSync( packageFilename ) ) {
            try {
                ...
                console.error( "For information, your current `package.json` file is not parsable!".red );
            }
        }
...
function yesno( caption, yesStep, noStep, defaultValue ) {
    ...
    input( [ caption, defaultValue ], function ( ans ) {
        ...
        if ( ans == 'Y' ) yesStep();
            ...
            console.log( "Please answer Y or N!".red );
