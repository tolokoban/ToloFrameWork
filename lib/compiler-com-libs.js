...
module.exports = function(source, components, scopes, output) {
    ...
    var libs = {
        ...
        fatal: function(msg, tree) {
            if (typeof tree !== 'undefined' && typeof tree.pos === 'number') {
        ...
        warning: function(msg, filename, content, pos) {
            ...
            console.log("------------------------------------------------------------".yellow);
            console.log("Warning in file ".yellow + filename.cyan);
    ...
    function compileChildren(root) {
        if (Array.isArray(root.children)) {
            // We create a new children array to avoid nodes of type VOID.
            var result = [];
            root.children.forEach(function (child) {
                libs.compile(child);
                if (child.type == Tree.VOID || typeof child.type === 'undefined') {
                    result.push.apply( result, child.children );
                } else {
                    result.push( child );
                }
            });
