...
var RX_VIEW_ATTRIB = /^($content|[a-z][a-z0-9]+(-[a-z0-9]+)*)$/;
var RX_INTEGER = /^[0-9]+$/;
var RX_STD_ATT = /^(\$content|[A-Za-z]+)$/;
var RX_TAG_NAME = /^[A-Z]+$/;
  ...
  if( Array.isArray( type ) ) {
    ...
    case 'any':
      if( typeof init !== 'undefined' ) {
        ...
        throw "Attribute `" + attName + "` must have a default value, because it is of type `any`!";
      }
      break;
    case 'boolean':
    case 'booleans':
...
function buildElementTagOn( def, code, varName ) {
  ...
  for( attName in def ) {
    ...
    code.section.ons.push(
      "try {",
      "  PM(" + varName + ").on(" + JSON.stringify(targetAttributeName) + ", " + behindFunctionName
        + ".bind(this));",
      "}",
      "catch( ex ) {",
      "  console.error( ex );",
...
function buildElementTagAttribsStandard( attribs, code, varName ) {
  ...
  for( attName in attribs ) {
    ...
    if( isSpecial( attValue, "bind" ) ) {
      ...
      code.section.elements.init.push(
        varName + keySyntax(attName) + " = _(" + JSON.stringify( attValue[1] ) + ");");
    }
    else {
      code.section.elements.init.push(
