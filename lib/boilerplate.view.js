...
function buildElement( def, code, _varName ) {
    ...
    try {
        ...
        if ( RX_TAG_NAME.test( type ) ) {
            ...
            //buildElementTagChildren( def, code, varName );
        } else if ( RX_CLS_NAME.test( type ) ) {
            buildElementCls( def, code, varName );
        } else throw Error( "Unknown element name: " + JSON.stringify( type ) + "!\n" +
...
function buildElementTagChildren( def, code, varName ) {
    const childOrChildren = def[ 1 ];
    if ( typeof childOrChildren === 'undefined' ) return;
    if ( isSpecial( childOrChildren ) ) {
        buildElementSpecialChild( childOrChildren, code, varName );
        return;
    }
    const
        children = Array.isArray( childOrChildren ) ? childOrChildren : [ childOrChildren ],
        toAdd = [];
    children.forEach( function ( child, index ) {
        try {
            if ( typeof child === 'string' || typeof child === 'number' ) {
            ...
            const message = `...in child #${index} - ${ex}`;
