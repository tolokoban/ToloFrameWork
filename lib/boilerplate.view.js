  ...
  try {
    initAttribs.forEach(function (initAttrib) {
      ...
      code.section.elements.init.push(
        varName + keySyntax(attName) + " = " + parseComplexValue( attValue, '          ' ) + ";");
    });
  }
  catch(ex) {
    throw `Error while defining tag's attribute "${attName}"!\n${ex}`;
  }
}
...
function buildInitAttribsForCls( initAttribs, code, varName, viewName ) {
  try {
    ...
    if( initAttribs.length === 0 ) {
      ...
      attValue = item[1];
      code.section.elements.define.push(
        "var " + varName + " = new " + viewName + "({ " + putQuotesIfNeeded(attName) + ": " + attValue + " });");
    }
    else {
      ...
      initAttribs.forEach(function (item, index) {
        try {
          attName = item[0];
          attValue = item[1];
          parsedValue = parseComplexValue( attValue, '          ' );
          out.push(
...
function parseComplexValue( value, indent ) {
  if( typeof indent === 'undefined' ) indent = '';

  var lines = recursiveParseComplexValue( value );
  console.info("[boilerplate.view] lines=", JSON.stringify(lines, null, '    ') );
...
function recursiveParseComplexValue( value ) {
  try {
    ...
    if( Array.isArray( value ) ) {
      ...
      if( value.length === 1 ) {
        val = recursiveParseComplexValue(value[0]);
      ...
      return [
        ...
        value.map(function( itm, idx ) {
          return recursiveParseComplexValue(itm)
            + (idx < value.length - 1 ? "," : "");
        }),
        "]"
      ];
      ...
      if( keys.length === 1 ) {
        val = recursiveParseComplexValue(value[keys[0]]);
        if( typeof val === 'string' ) {
      ...
      return [
        ...
        keys.map(function( key, idx ) {
          var val = recursiveParseComplexValue( value[key] );
          console.info("[boilerplate.view] key, idx, val=", key, idx, val);
