...
exports.generateCodeFrom = function( def, codeBehind, moduleName ) {
  ...
  out += "      throw Error('Error in XJS of " + JSON.stringify(moduleName) + ":\\n' + ex)\n";
  out += "    }\n";
  out += "  };\n";
...
function buildViewAttribs( def, code ) {
  ...
  for( attName in attribs ) {
    if( !RX_VIEW_ATTRIB.test( attName ) )
      throw Error(
        "Bad syntax for attribute's name: " + JSON.stringify( attName ) + "\n"
          + "Example of valid syntax: action, duration-visible, ...");
    attValue = attribs[attName];
    var camelCaseAttName = camelCase( attName );
    if( isSpecial( attValue ) ) buildViewAttribsSpecial( camelCaseAttName, attValue, code );
    else buildViewAttribsInit( camelCaseAttName, attValue, code );
  }
}
...
function buildViewAttribsSpecialCodeBehind( attName, functionBehindName, code ) {
  if( typeof functionBehindName !== 'string' )
    throw "The property \"behind\" of the attribute " + JSON.stringify( attName ) + " must be a string!";
  code.that = true;
  code.section.links.push(
    "pm.on( " + JSON.stringify( attName )
      + ", function(v) {",
    "  try {",
    "    CODE_BEHIND[" + JSON.stringify( functionBehindName ) + "].call( that, v );",
    "  }",
    "  catch( ex ) {",
...
function buildElement( def, code, varName ) {
  ...
  try {
    ...
    if( typeof def["view.id"] === 'string' ) {
      code.section.elements.define.push(
        "this.$elements" + keySyntax(def["view.id"]) + " = " + varName + ";");
    }
    code.section.elements.define.push(
...
function buildElementTagClassSwitcher( def, code, varName ) {
  ...
  for( attName in def ) {
    ...
    code.section.links.push(
      "new Link({",
      "  A:{obj: that, name: '" + camelCase(attValue[1]) + "'},",
      "  B:{action: function(v) {");
 ...
 * instance `attrib.|disabled:  {Link enabled}` means that  the attrib
...
function buildElementTagAttribSwitcher( def, code, varName ) {
  ...
  for( attName in def ) {
    ...
    attribs.forEach(function (att) {
      if( att.trim().length === 0 ) return;
      if( !RX_STD_ATT.test( att ) )
        throw "Invalid attribute name in attribute switcher: " + JSON.stringify(att) + "!";
    });
    ...
    if( isNonEmptyString( attribs[0] ) ) {
      ...
      code.functions["addAttribIfTrue"] = "(element, attribName, value) { "
        + "if( value ) $.att(element, attribName); else $.removeAtt(element, attribName); };";
      attribName = JSON.stringify(attribs[0]);
      code.section.links.push("  addAttribIfTrue( " + varName + ", " + attribName + ", v );");
    }
    if( isNonEmptyString( attribs[1] ) ) {
      code.requires.$ = "require('dom')";
      code.functions["addAttribIfFalse"] = "(element, attribName, value) { "
        + "if( value ) $.removeAtt(element, attribName); else $.att(element, attribName); };";
      attribName = JSON.stringify(attribs[1]);
      code.section.links.push("  addAttribIfFalse( " + varName + ", " + attribName + ", v );");
    }
    code.section.links.push("});");
  }
}
...
function buildElementTagClassSwitcherStar( items, code, varName ) {
  ...
  items.forEach(function (item, index) {
    ...
    code.functions[funcName] =
      ...
      + ".applyClass(CODE_BEHIND[" + JSON.stringify(behindFunction) + "].call(this), " + index + "); }";
...
/**
 * Return a string like this one:
 * new Link({
 *   A:{ obj: elementCheckbox, name: "value", open: false },
 *   B:{ obj: elementButton, name: "visible", open: true }
 * })
 */
    ...
    sources.forEach(function (source, index) {
      ...
      for( attName in def ) {
        ...
        if( attName === 'action' ) {
          ...
          output += attName + ": " + (attName === 'obj' ? attValue : JSON.stringify( attValue )) + ", ";
        }
      }
      result.push( output + "open: true }" );
      result.push("});");
    });
  ...
  for( attName in attribs ) {
    ...
    if( typeof attValue === 'string' ) {
      ...
      code.section.events.push(
        "$.on( " + varName + ", " + JSON.stringify( eventName ) + ", CODE_BEHIND["
          + JSON.stringify(attValue) + "].bind( this ) );");
    } else {
      code.section.events.push(
...
function buildElementTagAttribsStandard( attribs, code, varName ) {
  ...
  for( attName in attribs ) {
    ...
    if( isSpecial( attValue ) ) {
      var binding = clone( attValue );
      binding.obj = varName;
      binding.name = attName;
      code.section.links.push.apply(code.section.links, generateBind(binding, code));
    }
    else {
      code.section.elements.init.push(
