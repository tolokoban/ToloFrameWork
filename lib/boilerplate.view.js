...
exports.generateCodeFrom = function( def, codeBehind ) {
  ...
  out += "    //-----------------------\n";
  out += "    // Declare root element.\n";
  out += "    Object.defineProperty( this, '$', {value: " + rootElementName
...
function buildElementTagClassSwitcher( def, code, varName ) {
  ...
  for( attName in def ) {
    ...
    if( classesNames === '*' ) return buildElementTagClassSwitcherStar( dattValue, code, varName );
...
function getElementVarFromPath( path, root ) {
  ...
  var result = items.map(function(x, i) {
    if( i === 0 ) return x;
    if( i === items.length - 1 ) return keySyntax( x );
