...
Project.prototype.srcOrLibPath = function ( filename ) {
    ...
    console.info("[project] pathes=", JSON.stringify(pathes, null, '  '));
        ...
        dirs.forEach(function ( dir, idx ) {
            ...
            files.forEach(function ( filename ) {
                ...
                if ( !map[ filename ] ) {
                    ...
                    const name = ( filename.substr( 0, 1 ).toUpperCase() +
 ...
 * @param {string} filename - File without any path, but with an extension.
 * @return {string} If there is at leat one file found, return it.  Otherwise return `null`.
 */
function findHomonyme( path, filename ) {
    try {
        const basename = Util.removeExtension( filename ),
              extension = Util.getExtension( filename ),
              // Split against dots and dots.
              pieces = basename.split( '.' ).join( '/' ).split( '/' ),
        ...
        const candidates = precandidates.map( c => Path.resolve( path, `${c}.${extension}`) );
