...
function createIfNeededModule$(cfg, options) {
    ...
    if (PathUtils.isNewer(this.prjPath('package.json'), file)) {
        ...
        if (cfg.consts) {
            if (cfg.consts.all) {
                for( key in cfg.consts.all ) {
                    val = cfg.consts.all[key];
                    config.consts[key] = val;
                }
            }
            if (options.dev) {
                if (cfg.consts.debug) {
                    for( key in cfg.consts.debug ) {
                        val = cfg.consts.debug[key];
                        config.consts[key] = val;
                    }
                }
            } else {
                if (cfg.consts.release) {
                    for( key in cfg.consts.release ) {
                        val = cfg.consts.release[key];
                        config.consts[key] = val;
                    }
                }
            }
