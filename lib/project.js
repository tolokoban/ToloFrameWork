...
TagsManager = require("./tags-manager"),
Fatal = require( "./fatal" ),
FS = require( "fs" ),
Path = require( "path" ),
 ...
 */
Project.prototype.libPath = function ( _path ) {
    try {
        const path = typeof _path === 'string' && _path.substr( 0, this._srcDir.length ) == this._srcDir
              ? _path.substr( this._srcDir.length ) : _path || "";

        return Path.resolve( Path.join( this._libDir, path ) );
    } catch( ex ) {
        throw Error(`Error in Project.libPath(${JSON.stringify(_path)}):\n${ex}

    this._srcDir = ${JSON.stringify(this._srcDir)}
    this._libDir = ${JSON.stringify(this._libDir)}`);
    }
};
...
Project.prototype.createModule = function( moduleName ) {
    const pathes = [ this.srcPath() ];

    this._modulesPath.forEach( function ( modulePath ) {
        pathes.push( Path.resolve( modulePath ) );
    } );
    pathes.push( this.libPath() );

    return new Module( moduleName, pathes );
};

Project.prototype.createTagsManager = function() {
    return new TagsManager( this.tmpPath() );
};
        ...
        dirs.forEach(function ( dir, idx ) {
            ...
            files.forEach(function ( filename ) {
                ...
                if ( !map[ filename ] ) {
                    ...
                    let name = ( filename.substr( 0, 1 ).toUpperCase() +
...
Project.prototype.makeTest = function ( compiledFiles, specDir ) {
    ...
    const tags = this.createTagsManager();

    // Copy modules in `spec/mod`.
    allModules.forEach( function ( moduleName ) {
        //const js = new Source( this, `${moduleName}.js` );
        PathUtils.file( this.prjPath( `${specDir}/${moduleName}.js` ), tags.loadModJsZip( moduleName ) );
    }, this );
...
Project.prototype.linkForDebug = function ( filename, shiftPath ) {
    ...
    linkCSS.forEach(
        function ( item ) {
            ...
            resources.forEach(
                function ( resource ) {
                    const shortName = `css/${resource[ 0 ]}`;
                    const longName = resource[ 1 ];
    ...
    srcHTML.tag( "resources" ).forEach(function ( itm, idx, arr ) {
        let src = itm;
        let dst = src;
 ...
 */
Project.prototype.mkdir = function (...args) {
    try {
        let arg,
            items = [];
        ...
        Fatal.bubble(ex, `[project] mkdir(${args.map(x => JSON.stringify(x)).join(", ")})`);
    }
};
...
function compileHtmlFiles( htmlFiles, options ) {
    try {
        ...
        Fatal.bubble( ex, `[project] compileHtmlFiles( ${JSON.stringify(htmlFiles)} )` );
    }
}
