        ...
        FS.readdirSync( root ).forEach(function ( filename ) {
            if ( isDirectory( Path.join( root, filename ) ) ) {
                if ( !filters || !filter || filter.test( filename ) ) {
                    files.push(...findFiles( Path.join( root, filename ), filters, index + 1 ));
                }
            }
        });
    } else {
...
function mkdir(...args) {
    const path = Path.resolve( ...args );
    let curPath = "";
    const items = path.replace( /\\/ug, '/' ).split( "/" );
...
/**
 * @param {string} sourcePath - File name and path.
 * @param {string} referencePath - File name and path.
 * @return {bool} `true` if `sourcePath` exists and is more recent than `referencePath`./
